...
class DataPrepper:
        ...
        if response and len(response['hits']) > 0 and len(response['hits']['hits']) > 0:
            ...
            for hit in hits:
                ...
                for feature in features:
                    ...
                    ft_val = feature.get('value', 0)
                    if ft_name not in feature_results:
                        feature_results[ft_name] = []
                    feature_results[ft_name].append(ft_val)
    ...
    def __num_clicks(self, all_skus_for_query, test_sku):
        ...
        return len([x for x in all_skus_for_query if x==test_sku])
